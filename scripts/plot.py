...
def load_data(pkl_path, csv_path):
    ...
    if os.path.exists(pkl_path):
        ...
        print("Run collect_stats.py first to generate data files")
...
def generate_breakdown_per_benchmark_pdf(df, output_filename="memory_breakdown_by_workload.pdf"):
    """Generates a multi-page PDF showing memory read access breakdown for tiered memory configurations (writes excluded)."""
    print(f"\n--- Generating PDF 2: Per-Benchmark Read Access Breakdown ---")

    # Filter for tiered memory configurations (not dram_only or cxl_only)
    hybrid_df = df[~df['config'].isin(['dram_only', 'cxl_only'])].copy()
    if hybrid_df.empty:
        print("Warning: No tiered memory configurations found. Skipping breakdown PDF.")
        return
    ...
    with PdfPages(output_filename) as pdf:
        ...
        for workload_name, workload_df in hybrid_df.groupby('workload_key'):
            ...
            # 5 tiered memory configurations: interleaving, access_r1_1, access_r1_3, criticality_r1_1, criticality_r1_3
            tiered_configs = ['interleaving', 'access_r1_1', 'access_r1_3', 'criticality_r1_1', 'criticality_r1_3']
            ...
            for i, config_name in enumerate(tiered_configs):
