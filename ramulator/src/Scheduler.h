...
public:
    ...
    void set_type(const std::string& policy) {
        type = getTypeFromString(policy);
    }
...
private:
    ...
    function<ReqIter(ReqIter, ReqIter)> compare[int(Type::MAX)] = {
        ...
        // FRFCFS
        [this] (ReqIter req1, ReqIter req2) {
            bool ready1 = this->ctrl->is_ready_for_schedule(req1);
            bool ready2 = this->ctrl->is_ready_for_schedule(req2);
    ...
    Type getTypeFromString(const std::string& policy) {
        static const std::unordered_map<std::string, Type> policy_map = {
            {"FCFS", Type::FCFS},
            {"FRFCFS", Type::FRFCFS},
        ...
        if (it != policy_map.end()) {
            ...
            throw std::invalid_argument("Invalid scheduling policy: " + policy);
        }
    }
};
...
public:
    ...
    void update(typename T::Command cmd, const std::array<int, Request::ADDR_VEC_SIZE> &addr_vec, long clk)
    {
    ...
    int get_hits(const std::array<int, Request::ADDR_VEC_SIZE> &addr_vec, const bool to_opened_row = false)
    {
        auto begin = addr_vec.begin();
        auto end = begin + int(T::Level::Row);
    ...
    int get_open_row(const std::array<int, Request::ADDR_VEC_SIZE> &addr_vec) {
        auto begin = addr_vec.begin();
        auto end = begin + int(T::Level::Row);
