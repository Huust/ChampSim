...
static std::array<int, Request::ADDR_VEC_SIZE>
get_offending_subarray(DRAM<SALP> *channel, std::array<int, Request::ADDR_VEC_SIZE> &addr_vec) {
    int sa_id = 0;
    ...
    std::array<int, Request::ADDR_VEC_SIZE> offending = addr_vec;
...
bool Controller<SALP>::is_ready(list<Request>::iterator req){
    ...
    if (cmd == SALP::Command::PRE_OTHER){
        ...
        std::array<int, Request::ADDR_VEC_SIZE> addr_vec = get_offending_subarray(channel, req->addr_vec);
