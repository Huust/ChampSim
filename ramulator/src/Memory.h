...
#else
  ScalarStat incoming_requests_per_channel;  // Each memory object has a single channel, thus changed to ScalarStat
  ScalarStat incoming_read_reqs_per_channel; // Each memory object has a single channel, thus changed to ScalarStat
  string channel_id; // Need to keep the track of channel_id
#endif
...
public:
          ...
          addr_bits(int(T::Level::MAX))
    {
        ...
        if(configs.contains("gem5_size")){
            // Check if the DRAM size matches the configuration file
            printf("Gem5 DRAM size: %ld\n", stol(configs["gem5_size"]));
            assert(stol(configs["gem5_size"]) == max_address && "DRAM size does not match the configuration file");
        }
        num_dram_cycles
            .name("dram_cycles")
        ...
        in_queue_write_req_num_avg
            ...
            ;
#else
        assert(configs.contains("gem5_channel_id"));
        channel_id = "_" + configs["gem5_channel_id"];
        ...
        assert(configs.contains("gem5_size"));
        // Check if the DRAM size matches the configuration file
        printf("Gem5 DRAM size: %ld\n", stol(configs["gem5_size"]));
        assert(stol(configs["gem5_size"]) == max_address && "DRAM size does not match the configuration file");
        
        num_dram_cycles
            .name("dram_cycles" + channel_id)
            .desc("Number of DRAM cycles simulated")
            .precision(0)
            ;
        num_incoming_requests
            .name("incoming_requests" + channel_id)
            .desc("Number of incoming requests to DRAM")
            .precision(0)
            ;
        num_read_requests
            .init(configs.get_core_num())
            .name("read_requests" + channel_id)
            .desc("Number of incoming read requests to DRAM per core")
            .precision(0)
            ;
        num_write_requests
            .init(configs.get_core_num())
            .name("write_requests" + channel_id)
            .desc("Number of incoming write requests to DRAM per core")
            .precision(0)
            ;
        incoming_requests_per_channel
            .name("incoming_requests_per_channel" + channel_id)
            .desc("Number of incoming requests to each DRAM channel")
            .precision(0)
            ;
        incoming_read_reqs_per_channel
            .name("incoming_read_reqs_per_channel" + channel_id)
            .desc("Number of incoming read requests to each DRAM channel")
            .precision(0)
            ;

        ramulator_active_cycles
            .name("ramulator_active_cycles" + channel_id)
            .desc("The total number of cycles that the DRAM part is active (serving R/W)")
            .precision(0)
            ;
        physical_page_replacement
            .name("physical_page_replacement" + channel_id)
            .desc("The number of times that physical page replacement happens.")
            .precision(0)
            ;

        in_queue_req_num_sum
            .name("in_queue_req_num_sum" + channel_id)
            .desc("Sum of read/write queue length")
            .precision(0)
            ;
        in_queue_read_req_num_sum
            .name("in_queue_read_req_num_sum" + channel_id)
            .desc("Sum of read queue length")
            .precision(0)
            ;
        in_queue_write_req_num_sum
            .name("in_queue_write_req_num_sum" + channel_id)
            .desc("Sum of write queue length")
            .precision(0)
            ;
        in_queue_req_num_avg
            .name("in_queue_req_num_avg" + channel_id)
            .desc("Average of read/write queue length per memory cycle")
            .precision(6)
            ;
        in_queue_read_req_num_avg
            .name("in_queue_read_req_num_avg" + channel_id)
            .desc("Average of read queue length per memory cycle")
            .precision(6)
            ;
        in_queue_write_req_num_avg
            .name("in_queue_write_req_num_avg" + channel_id)
            .desc("Average of write queue length per memory cycle")
            .precision(6)
            ;
#endif
    ...
    bool send(Request req)
    {
        ...
        assert(req.addr_vec[0] == stoi(channel_id.substr(1)));
        // We only have a single controller in this case
    ...
    void apply_mapping(long addr, std::array<int, Request::ADDR_VEC_SIZE> &addr_vec) {
        int *sz = spec->org_entry.count;
...
#else
      for (auto ctrl : ctrls) {
        long read_req = long(incoming_read_reqs_per_channel.value());
        ctrl->finish(read_req, dram_cycles);
      }
#endif
