...
void run_dramtrace(const Config& configs, Memory<T, Controller>& memory, const char* tracename) {
    ...
    auto read_complete = [&latencies,&requestsInFlight](Request& r){
      latencies[r.depart - r.arrive]++;
      assert(requestsInFlight >= 0);
      requestsInFlight--;
    };
    ...
    while (!end || memory.pending_requests()){
        ...
        memory.tick();
        clks ++;
        RamulatorStats::curTick++; // memory clock, global, for Statistics
    }
...
void run_cputrace(const Config& configs, Memory<T, Controller>& memory, const std::vector<const char *>& files)
{
    ...
    for(long i = 0; is_warming_up; i++){
        proc.tick();
        RamulatorStats::curTick++;
        if (i % cpu_tick == (cpu_tick - 1))
            for (int j = 0; j < mem_tick; j++)
                memory.tick();

        is_warming_up = false;
        for(unsigned c = 0; c < proc.cores.size(); c++){
    ...
    for (long i = 0; ; i++) {
        if (((i % tick_mult) % mem_tick) == 0) { // When the CPU is ticked cpu_tick times,
            ...
            proc.tick();
            RamulatorStats::curTick++; // processor clock, global, for Statistics
...
int main(int argc, const char *argv[])
{
    ...
    if (strcmp(argv[trace_start], "--stats") == 0) {
      RamulatorStats::statlist.output(argv[trace_start+1]);
      stats_out = argv[trace_start+1];
      trace_start += 2;
    } else {
      RamulatorStats::statlist.output(standard+".stats");
